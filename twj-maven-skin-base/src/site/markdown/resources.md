...
Tawja Maven Skin utilises content delivery networks (CDN) for a number of CSS and JavaScript
...
To support such deployments of Tawja Maven Skin, the configuration allows using local resources.
...
Bootstrap and jQuery files. Tawja Maven Skin does not package the default files at the moment, mainly
...
[customiser][bootstrap-cust] or [Bootswatch][bootswatch2] and use them with the Tawja Maven Skin.
...
-   **`css/bootstrap.min.css`** - Tawja Maven Skin currently uses Bootstrap 3.3.1
...
Also, it can be used for custom (local or remote) versions of hard-coded Tawja Maven Skin resources.
...
Note: the `<endContent>` element is specific to Tawja Maven Skin and needs to be placed within the
`<custom><twjSkin>` element.
